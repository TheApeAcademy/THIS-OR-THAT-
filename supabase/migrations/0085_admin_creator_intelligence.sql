-- Integration step (I1, safe layer): an audit log for admin actions,
-- admin dashboard metrics, per-comparison creator insights, and
-- recency-weighted Preference DNA.
--
-- This deliberately does NOT include the branch's comparisons.view_count /
-- increment_comparison_view() — main already added the exact same column
-- and an equivalent function (0058_comparison_view_count.sql) before this
-- branch existed. Dropped per the I2/I4 dedup rather than carried over;
-- get_comparison_insights below still works unchanged since it never
-- referenced view_count in the first place (it reads votes.created_at).

-- ============================================================
-- audit log — one row per privileged action, written alongside the
-- mutation itself in lib/actions/admin.ts.
-- ============================================================
...
-- ============================================================
-- admin dashboard metrics — DAU/WAU/MAU come from user_sessions (login
-- history), the only real per-day activity signal this repo has;
...
-- real number lie. handle_new_vote's base is still the shared 0001
-- definition, untouched by main, so this layers on cleanly.
-- ============================================================
