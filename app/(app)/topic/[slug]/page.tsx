...
export default async function TopicPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: taggedRows }, hiddenAuthorIds, { data: myFollow }] = await Promise.all([
    supabase
    ...
    user
      ? supabase.from("topic_follows").select("topic_id").eq("topic_id", topic.id).eq("user_id", user.id).maybeSingle()
  ...
  return (
    <div className="mx-auto max-w-md space-y-6 px-4 py-4">
      <div className="flex items-center justify-between">
        ...
        {user && <TopicFollowButton topicId={topic.id} initialFollowing={!!myFollow} />}
      </div>
