...
export function AskProfileQuestion() {
  ...
  const ask = async () => {
    ...
    try {
      ...
      if (error) {
        setNote("Something went wrong asking that.");
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-surface-raised p-4">
      <p className="text-sm font-semibold text-text-secondary">Ask my profile anything</p>
      <div className="flex gap-2">
        <input
          value={question}
          onChange={(e) => setQuestion(e.target.value)}
          placeholder="e.g. Do I prefer BMW or Mercedes?"
          maxLength={300}
